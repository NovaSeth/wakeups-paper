...
This paper proposes one such alternative: the **wakeup loop**. The author defines it as a harness configuration in which the model is invoked on a wall-clock cadence (e.g., every ten minutes), receives the current time and the delta since its last invocation as part of its input, is informed that producing no operation is a fully legal cycle outcome, and may consult fresh subagent instances with bounded prompts and clean context. The wakeup loop is not a model architecture. It is not a fine-tuning regime. It is a harness pattern - a small configuration change to existing agent infrastructure - and that smallness is part of the argument: the cost of trying it is low, while the design space it opens is, the author believes, larger than has so far been recognized. The recognition that *the three separate harness assumptions enumerated above compose a single previously unnamed pattern, rather than three independent constraints*, is the central conceptual contribution of this paper, and it is the author's. The reasoning about why a model whose inference loop has no innate temporal abstraction nonetheless needs one - and why "do nothing" deserves to be a first-class output rather than absence-of-output - sits at the core of the argument. The wakeup loop is the design that follows.
...
The model snapshots used in this work - Claude Opus 4.7 and Claude Sonnet 4.6 - were developed by Anthropic. The night-1 replication was carried out using Claude Code's Agent tool to dispatch ten parallel subagents under a unified prompt designed by the author; the orchestrator's role and constraints are documented in the supplementary `replication-night-1/` archive. The intellectual structure of the paper - the recognition that three separate harness constraints compose a single previously unnamed pattern, the framing of "doing nothing" as a first-class cycle outcome rather than as failure or refusal, and the reasoning about why a temporal abstraction matters for a model that has no innate sense of time - is the author's, developed across the pilot night of 26-27 April 2026 and the analysis that followed.
